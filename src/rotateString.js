...
*/
function rotateString(toBeRotated) {
    var rotatedArray = toBeRotated.map(rotateSingleString);
    var rotatedStrings = rotatedArray.join(" ");
    return rotatedStrings;
}
function rotateSingleString(array) {
    var operation = array[0];
    var originalStringAsArray = array[1].split("");
    if (operation >= 0) {
        var movedLetters = originalStringAsArray.splice(0, operation);
        var currentRotation = originalStringAsArray.concat(movedLetters);
        var currentRotatedString = currentRotation.join("");
        return currentRotatedString;
    }
    else {
        var movedLetters = originalStringAsArray.splice(operation);
        var currentRotation = movedLetters.concat(originalStringAsArray);
        var currentRotatedString = currentRotation.join("");
        return currentRotatedString;
    }
}
